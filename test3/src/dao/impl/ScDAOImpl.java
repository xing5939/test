...
public class ScDAOImpl implements ScDAO{
	private static final Logger log = LoggerFactory.getLogger(ScDAOImpl.class);
	// property constants
	...
	public Sc findById(java.lang.Integer id) {
		log.debug("getting Sc instance with id: " + id);
		try {
			Sc instance = (Sc) getCurrentSession().get("vo.Sc", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Sc> findByExample(Sc instance) {
		log.debug("finding Sc instance by example");
		try {
			List<Sc> results = (List<Sc>) getCurrentSession()
					.createCriteria("vo.Sc").add(create(instance)).list();
	...
	public Sc merge(Sc detachedInstance) {
		...
		try {
			Sc result = (Sc) getCurrentSession().merge(detachedInstance);
	...
	public static ScDAOImpl getFromApplicationContext(ApplicationContext ctx) {
		return (ScDAOImpl) ctx.getBean("ScDAO");
	}
